...
fn main() {
    ...
    let output = resolved_factory
        .bots
        .into_iter()
        .find(|(_bot_id, values)| *values == (17, 61))
        .map(|(bot_id, _values)| bot_id)
        .expect("Could not find designated bot");
    println!("{}", output);
}
